...
def set_resolvconf_to_localhost():
    with open(RESOLV_CONF_PATH, "w") as file1:
        file1.writelines(
            [
                "# This is generated by Pardus Parental Control app. Please do not change. This file is locked by Pardus Parental Control app.",
                "nameserver 127.0.0.1",
            ]
        )

    subprocess.run(["chattr", "+i", "/etc/resolv.conf"])  # block changing
...
def disable_smartdns_service():
    return subprocess.run(["systemctl", "disable", "smartdns-rs"])


def enable_smartdns_service():
    return subprocess.run(["systemctl", "enable", "smartdns-rs"])
