...
def new(title, subtitle="", icon_name="", gicon=None, on_activated=None, on_deleted=None, on_edited=None, user_data=None, activatable_widget=None):
    action_row = None
    if on_edited:
        action_row = Adw.EntryRow(
            title=title, text=title, show_apply_button=True, activates_default=True)
    ...
    if icon_name:
        action_row.add_prefix(Gtk.Image(
            icon_name=icon_name,
            pixel_size=32
        ))
    elif gicon:
    ...
    if on_deleted:
        ...
        btn_delete.connect(
            "clicked", on_deleted, action_row, user_data)
